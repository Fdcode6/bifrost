...
} from "@/app/workspace/logs/layoutConfig";
...
import { formatLayerLabel, RequestGroupDisplayState } from "../requestGrouping";
...
interface LogDetailSheetProps {
	log: LogEntry | null;
	groupState: RequestGroupDisplayState | null;
...
export function LogDetailSheet({
	...
	const requestSummaryTarget = groupState?.finalTarget;
	const selectedAttemptId = displayLog.id;

	return (
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className={LOGS_DETAIL_SHEET_CLASS}>
				{!isFullDataReady ? (
					...
					<>
						...
						<div className={LOGS_DETAIL_BODY_CLASS}>
							<div className="-mt-6 space-y-4 rounded-sm border px-6 py-4">
								<div className="space-y-4">
									<BlockHeader title="Request Summary" />
									<div className="grid gap-4 md:grid-cols-4">
										<LogEntryDetailsView
											className="w-full"
											label="View"
											value={<Badge variant="outline">{sheetMode === "request" ? "Request View" : "Attempt View"}</Badge>}
										/>
										<LogEntryDetailsView
											className="w-full"
											label="Status"
											value={groupState?.finalAttemptVisible ? (groupState.finalStatus ?? "Unknown") : "Partial"}
										/>
										<LogEntryDetailsView className="w-full" label="Visible Attempts" value={groupState?.visibleAttemptCount ?? 1} />
										<LogEntryDetailsView
											className="w-full"
											label="Final Layer"
											value={requestSummaryTarget ? requestSummaryTarget.layerLabel : "Unavailable in current page"}
										/>
									</div>
									<div className="grid gap-4 md:grid-cols-2">
										<LogEntryDetailsView
											...
											label="Final Target"
											value={
												requestSummaryTarget
													? `${requestSummaryTarget.provider} / ${requestSummaryTarget.model}`
													: "Unavailable in current page"
											}
										/>
										<LogEntryDetailsView
											className="w-full"
											label="Group ID"
											value={groupState?.groupId ?? displayLog.group_id ?? displayLog.parent_request_id ?? displayLog.id}
										/>
									</div>
									<div className="space-y-2">
										<div className="text-sm font-medium">Attempt Timeline</div>
										<div className="space-y-2">
											{(groupState?.attempts ?? [displayLog]).map((attempt) => (
												<div
													key={attempt.id}
													className={`flex items-center justify-between gap-3 rounded-sm border px-3 py-2 ${attempt.id === selectedAttemptId ? "border-foreground" : ""}`}
												>
													<div className="min-w-0">
														<div className="flex flex-wrap items-center gap-2">
															<Badge variant="outline">Attempt {attempt.attempt_sequence ?? 1}</Badge>
															<Badge variant={attempt.status === "success" ? "default" : "secondary"}>{attempt.status}</Badge>
															<Badge variant="outline">{formatLayerLabel(attempt.route_layer_index ?? null)}</Badge>
														</div>
													...
													<div className="text-muted-foreground text-right text-xs">
														<div>{moment(attempt.timestamp).format("YYYY-MM-DD HH:mm:ss")}</div>
														<div>{attempt.latency != null ? `${attempt.latency.toLocaleString()}ms` : "Latency unavailable"}</div>
													</div>
												</div>
											))}
										</div>
										{groupState?.isPartial && (
											<div className="text-muted-foreground text-xs">Only the attempts visible on the current page are available here.</div>
										)}
									</div>
								</div>
								<DottedSeparator />
							...
							{displayLog.routing_engine_logs && (
								<CollapsibleBox title="Routing Decision Logs" onCopy={() => displayLog.routing_engine_logs || ""}>
									<div className={`${LOGS_DETAIL_SCROLL_AREA_CLASS} break-words whitespace-pre-wrap`}>{displayLog.routing_engine_logs}</div>
								</CollapsibleBox>
							)}
							{toolsParameter && (
								<CollapsibleBox title={`Tools (${displayLog.params?.tools?.length || 0})`} onCopy={() => toolsParameter}>
									<CodeEditor
										...
										maxHeight={LOGS_DETAIL_CODE_MAX_HEIGHT}
										wrap={true}
										code={toolsParameter}
										lang="json"
										readonly={true}
							...
							{displayLog.params?.instructions && (
								<CollapsibleBox title="Instructions" onCopy={() => displayLog.params?.instructions || ""}>
									<div className={`${LOGS_DETAIL_SCROLL_AREA_CLASS} break-words whitespace-pre-wrap`}>{displayLog.params.instructions}</div>
								</CollapsibleBox>
							)}
									...
									{displayLog.error_details?.error.message && (
										<>
											...
											<CollapsibleBox title="Error" onCopy={() => displayLog.error_details?.error.message || ""}>
												<div className={`${LOGS_DETAIL_SCROLL_AREA_CLASS} break-words whitespace-pre-wrap`}>
													{displayLog.error_details.error.message}
												</div>
											</CollapsibleBox>
										</>
