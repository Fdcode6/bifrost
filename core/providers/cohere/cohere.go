...
func (provider *CohereProvider) ChatCompletionStream(ctx *schemas.BifrostContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.BifrostChatRequest) (chan *schemas.BifrostStreamChunk, *schemas.BifrostError) {
	...
	if err != nil {
		...
		if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
			return nil, providerUtils.EnrichError(ctx, providerUtils.NewBifrostTimeoutErrorWithTimeoutSeconds(providerUtils.TimeoutSecondsFromFastHTTPClient(provider.client), err, providerName), jsonBody, nil, sendBackRawRequest, sendBackRawResponse)
		}
