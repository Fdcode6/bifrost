...
const (
	ErrProviderRequestTimedOut      = "request timed out. You can increase it by setting the default_request_timeout_in_seconds in the network_config or in UI - Providers > Provider Name > Network Config."
...
func ProviderRequestTimedOutMessage(timeoutSeconds int) string {
	if timeoutSeconds <= 0 {
		return ErrProviderRequestTimedOut
	}
	...
	return fmt.Sprintf(
		"request timed out after %d %s. You can increase it by setting the default_request_timeout_in_seconds in the network_config or in UI - Providers > Provider Name > Network Config.",
		timeoutSeconds,
		unit,
	)
}
