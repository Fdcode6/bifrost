...
export class LogsPage extends BasePage {
	...
	readonly finalSuccessDistributionCard: Locator;
	readonly requestGroupRows: Locator;
	...
	constructor(page: Page) {
		...
		// Main elements
		this.logsTable = page.getByTestId("logs-table");
		...
		this.statsCards = page.getByTestId("stats-cards").or(page.locator("text=Request Success Rate").locator("..").locator(".."));
		this.finalSuccessDistributionCard = page.getByTestId("logs-final-success-distribution-card");
		this.requestGroupRows = page.locator('[data-testid^="logs-request-group-"]');

		// Filter elements - filters are inside a popover opened by the Filters button
		this.providerFilter = page.locator('[data-testid="filter-provider"]').or(page.locator("button").filter({ hasText: /Provider/i }));
		this.modelFilter = page.locator('[data-testid="filter-model"]').or(page.locator("button").filter({ hasText: /Model/i }));
		this.statusFilter = page.locator('[data-testid="filter-status"]').or(page.locator("button").filter({ hasText: /Status/i }));
		this.searchInput = page.locator('[data-testid="filter-search"]').or(page.getByPlaceholder("Search logs"));
		this.dateRangePicker = page.locator('[data-testid="filter-date-range"]').or(page.locator("button").filter({ hasText: /Last/i }));
		this.liveToggle = page.locator('[data-testid="live-toggle"]').or(page.getByRole("button", { name: /Live updates/i }));

		// Table elements - exclude the "Listening for logs" row which is not a data row
		this.tableRows = this.logsTable
			.locator("tbody tr")
			.filter({ hasNot: page.locator("text=Listening for logs") })
			.filter({ hasNot: page.locator("text=Live updates paused") })
			.filter({ hasNot: page.locator("text=Not connected") })
	...
	private async loginIfRequired(): Promise<void> {
		...
		const loginVisible =
			(await usernameInput
				...
				.then(() => true)
				.catch(() => false)) && (await passwordInput.isVisible().catch(() => false));
		if (!loginVisible) {
			return;
		}
		...
		if (!username || !password) {
			throw new Error("Login page is shown but admin credentials are not available in the environment.");
		}
		...
		await usernameInput.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
	}
	...
	async goToNextPage(): Promise<void> {
		...
		await btn.click();
		await this.page.waitForURL((url) => new URL(url).searchParams.get("offset") === String(expectedOffset), { timeout: 10000 });
		await waitForNetworkIdle(this.page);
	}
	...
	async areStatsVisible(): Promise<boolean> {
		return await this.statsCards.isVisible().catch(() => false);
	}
